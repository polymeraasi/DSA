...
//
// original name of the file: datastructures.cc
//
...
bool Datastructures::add_publication(PublicationID id, const Name &name, Year year, const std::vector<AffiliationID> &affiliations)
{
   ...
   for (const auto& affiliation : affiliations) {
        add_affiliation_to_publication(affiliation, id);
    }
...
// This function was modified from prg1 inorder to get the graph connections and weights updated.
bool Datastructures::add_affiliation_to_publication(AffiliationID affiliationid, PublicationID publicationid)
{
    ...
    if (find_publication != publications_.end() && find_affiliation != affiliations_.end()) {
        ...
        for (const AffiliationID & id : find_publication->second.affiliations) {
            auto iter = affiliations_.find(id);

            if (id != affiliationid) {
                bool connected = false;

                for (auto& connection : find_affiliation->second.connections) {
                    if (connection.aff2 == id) {
                        connected = true;
                        connection.weight += 1;
                    }
                }

                for (auto& connection : iter->second.connections) {
                    if (connection.aff2 == affiliationid) {
                        connection.weight += 1;
                    }
                }
                ...
                if (!connected) {
                    Weight weight = 1;
                    Connection connection_first = {affiliationid, id, weight};
                    Connection connection_second = {id, affiliationid, weight};

                    if (iter != affiliations_.end()) {
                        find_affiliation->second.connections.push_back(connection_first);
                        iter->second.connections.push_back(connection_second);
                    }
                }
            }
        }
...
// Returns the connections from the given start point to all connected endpoints. 
// If there are no connections or given starting point, an empty vector is returned. Connections can be restored in any order.
// However, the following must apply to a single connection (i.e. the Connection structure):
// The first Affiliation (aff1) is given the ID in the structure, and the second Affiliation (aff2) is the affiliation connected to it.
std::vector<Connection> Datastructures::get_connected_affiliations(AffiliationID id)
{
    // connections -> every conection is already there

    auto find_affiliation = affiliations_.find(id);
    std::vector<Connection> no_affs = {};

    if (find_affiliation == affiliations_.end()) {
        return no_affs;
    } else {
        return find_affiliation->second.connections;
    }
}

// Returns all connections. In connections, the smaller Affiliation id should appear first. 
// There must be no duplicates in the vector, but connections may be returned in any order.
// If there are no connections, an empty vector is returned.
// The following must apply to a single connection: in struct, the smaller ID comes first, i.e. aff1 < aff2.
std::vector<Connection> Datastructures::get_all_connections()
{
    std::vector<Connection> all_connections;
    for ( const auto& [id, affiliation] : affiliations_) {
        for (const auto& connection : affiliation.connections) {
            if (connection.aff2 > connection.aff1) {
                if (std::find(all_connections.begin(), all_connections.end(), connection) == all_connections.end()) {
...
// Route search between two nodes (source -> target) according to the BFT algorithm.
// Already visited nodes are kept track of using the visited set, the queue is used in breadth-first search.
// For each unvisited node, create a new route by attaching the connection to the current route and
// enqueuing a new route and exploring this new route until the target node is found.
// If no route, or start or end affiliation was found, an empty vector is returned.
Path Datastructures::get_any_path(AffiliationID source, AffiliationID target)
{
    // std::vector<Connection> path;
    Path path;
    auto start = affiliations_.find(source);
    auto end = affiliations_.find(target);

    std::unordered_set<AffiliationID> visited;
    std::queue<std::vector<Connection>> q;

    if (start == affiliations_.end() || end == affiliations_.end()) {
        return path;
    }

    for (const auto& connection : start->second.connections) {
        q.push({connection});
    }
    ...
    while (!q.empty()) {
        ...
        q.pop();
        Connection current_connection = current_path.back();
        AffiliationID current_aff = current_connection.aff2;

        if (current_aff == target) {
            return current_path;
        }

        visited.insert(current_aff);

        for (const auto& connection : affiliations_[current_aff].connections) {
            AffiliationID next_aff = connection.aff2;

            if (visited.find(next_aff) == visited.end()) {
                visited.insert(next_aff);
