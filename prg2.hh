...
// 240 lines of code. I have analyzed the efficiency of each function and made the private interface myself.
//
// Original name of the file datastructures.hh
...
public:
    ...
    // Short rationale for estimate: Returning size of an unordered map.
    ...
    // Estimate of performance: O(n)
    // Short rationale for estimate: For-loops is used.
    ...
    // Estimate of performance: O(n)
    // Short rationale for estimate: Worst case of std::find() for a map. Otherwise maybe O(1), since returning a vector.
    std::vector<Connection> get_connected_affiliations(AffiliationID id);

    // Estimate of performance: O(n^3)
    // Short rationale for estimate: Using two for-loops inside one another, with addition to std::find().
    std::vector<Connection> get_all_connections();

    // Estimate of performance: O(n^2)
    // Short rationale for estimate: For-loop inside a while-loop.
